      ...
      <div className="flex flex-col md:flex-row gap-3 items-stretch relative" ref={containerRef}>
          ...
          <AnimatePresence>
            {showSuggestions && (searchSuggestions.length > 0 || recentSearches.length > 0) && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: 5 }}
                className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 overflow-hidden"
              >
                {/* Suggestions List */}
        ...
        <div className="relative shrink-0">
          <select
            ...
            className="appearance-none border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl pl-4 pr-10 py-3 text-sm font-bold text-slate-600 dark:text-slate-300 focus:outline-none cursor-pointer outline-none w-full md:w-44"
          >
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-xs py-1 select-none">
        ...
        {popularSearches.map(term => (
          <button
            ...
            className="text-[10px] font-bold border border-slate-200 dark:border-slate-800 hover:border-primary/20 bg-white dark:bg-slate-900 hover:bg-primary/5 text-slate-600 dark:text-slate-400 hover:text-primary px-3 py-1 rounded-full transition-all cursor-pointer"
          >
            {term}
          </button>
        ))}
