      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Warning */}
            <div className="bg-amber-50 dark:bg-amber-950/60 p-2.5 text-xs font-medium text-amber-700 dark:text-amber-400 flex items-center justify-center gap-2 border-b border-amber-100/50 dark:border-amber-900/50">
              <ShieldAlert className="h-3.5 w-3.5" /> AI can make mistakes. Verify official links.
            </div>

            {/* Body */}
            <div className="flex-1 p-5 overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50">
              <motion.div 
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex flex-col gap-4"
              >
                <div className="bg-card border border-border p-4 rounded-2xl rounded-tl-sm text-sm text-foreground shadow-sm">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  {['Government jobs after 12th', 'Scholarships for girl child', 'Latest farmer schemes'].map((prompt, i) => (
                    <motion.button 
                      ...
                      className="text-left text-sm bg-card dark:bg-slate-800 border border-primary/20 hover:border-primary/50 text-primary py-2.5 px-4 rounded-xl flex items-center justify-between shadow-sm transition-colors cursor-pointer"
                    >
            ...
            <div className="p-4 border-t border-border bg-card">
              <form onSubmit={handleSubmit} className="flex gap-2 relative">
                <input 
                  ...
                  className="flex-1 bg-slate-100 dark:bg-slate-800 border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-xl pl-4 pr-12 py-3 text-sm outline-none transition-all text-slate-800 dark:text-slate-100"
                />
