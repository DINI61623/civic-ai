...
export default function SchemesPage() {
  ...
  const handleFilterChange = useCallback((filtered: Scheme[], q: string) => {
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl relative">
      ...
      {/* Title */}
      <div className="mb-8">
        <h1 className="text-3xl md:text-4.5xl font-extrabold mb-2 text-slate-800 dark:text-white tracking-tight">Government Schemes</h1>
        <p className="text-foreground-muted text-sm md:text-base max-w-2xl leading-relaxed">Discover welfare programs, grants, subsidies, and citizen support yojanas tailored to your demographic profile.</p>
      </div>
        ...
        <>
          {filteredSchemes.length === 0 ? (
            <div className="bg-white dark:bg-slate-850 border border-slate-200 p-12 rounded-3xl text-center shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
              ...
              <h3 className="font-bold text-foreground text-lg mb-2">No welfare schemes match your demographic filters</h3>
              <p className="text-sm text-foreground-muted mb-6">Try changing your categories or domicile state to discover matching government yojanas.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSchemes.map((scheme) => {
                const status = getStatusBadge(scheme.application_end_date, scheme.application_start_date);
                ...
                return (
                  ...
                  >
                    ...
                    <h2 className="text-base font-extrabold text-slate-800 dark:text-white group-hover:text-primary transition-colors leading-snug mb-2 line-clamp-2 pl-1.5">
                      <HighlightText text={scheme.title} search={searchQuery} />
                    </h2>

                    <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 mb-4 pl-1.5 font-medium">
                      <HighlightText text={scheme.description || "Active government subsidy and citizen assistance schemes."} search={searchQuery} />
                    </p>
                    ...
                    <div className="grid grid-cols-3 gap-1 bg-slate-50 p-3 rounded-2xl border border-slate-100 dark:bg-slate-900/40 dark:border-slate-800 text-center mb-5 mt-auto select-none">
                      <div>
                        ...
                        <div className="text-[10.5px] font-extrabold text-slate-700 dark:text-slate-300 truncate">{scheme.category}</div>
                      </div>
                      <div>
                        <div className="text-[9px] uppercase font-bold text-slate-400">Benefits</div>
                        <div className="text-[10.5px] font-extrabold text-slate-700 dark:text-slate-300 truncate">{scheme.benefits}</div>
                      </div>
                      <div>
                        ...
                        <div className="text-[10.5px] font-extrabold text-rose-500 truncate">
                          {scheme.application_end_date ? new Date(scheme.application_end_date).toLocaleDateString('en-IN', {day:'numeric', month:'short'}) : 'Always Open'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 select-none">
                      ...
                      {scheme.apply_link && status.label !== 'Closed' && (
                        <a href={scheme.apply_link?.startsWith('http') ? scheme.apply_link : `https://${scheme.apply_link}`} target="_blank" rel="noopener noreferrer" className="flex-1">
