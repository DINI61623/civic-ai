...
export default function SettingsPage() {
  ...
  // language state is now managed globally by LanguageProvider; local shadow for profile form
    ...
    async function loadSettings() {
      try {
        ...
        if (type === 'farmer') {
          ...
          if (fp.language) setLocalLanguage(fp.language);
  ...
  const handleUpdateLanguage = async (lang: SupportedLanguage) => {
    // Update global language context immediately — all pages update in real time
  ...
  const tabs = [
    ...
    { id: 'theme', name: t('settings.theme'), icon: Moon },
    { id: 'privacy', name: t('settings.privacy'), icon: Shield },
    { id: 'account', name: t('settings.account'), icon: Trash2 },
  ] as const;
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl relative select-none">
      ...
      >
        <ArrowLeft className="h-4 w-4 group-hover:-translate-x-0.5 transition-transform" /> {t('settings.back_to_dashboard')}
      </button>

      <div className="mb-8">
        <h1 className="text-2xl md:text-3.5xl font-black text-slate-800 dark:text-white tracking-tight">{t('settings.title')}</h1>
        <p className="text-sm text-slate-500 font-medium mt-1">{t('settings.subtitle')}</p>
      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 outline-none focus:border-primary dark:focus:border-primary placeholder:text-slate-400 dark:placeholder:text-slate-600"
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="mt-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                    <p className="text-[10px] uppercase font-extrabold tracking-wider text-slate-400 mb-2">Live Preview</p>
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{t('home.hero.title')}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('home.hero.subtitle')}</p>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="max-w-xl pt-2">
                      ...
                      <select
                        ...
                        className="w-full sm:w-1/2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 outline-none focus:border-primary cursor-pointer"
                      >
