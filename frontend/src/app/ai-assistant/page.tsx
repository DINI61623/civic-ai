...
import { 
  ...
  Volume2, VolumeX, Languages, Loader2
} from 'lucide-react';
...
function AIAssistantContent() {
  ...
  // Persistence for selected language
  useEffect(() => {
    const savedLang = localStorage.getItem('civicai_tts_lang');
    if (savedLang) {
  ...
  const handleLangChange = (lang: string) => {
    ...
    localStorage.setItem('civicai_tts_lang', lang);
    setIsLangDropdownOpen(false);
    ttsService.stop();
  ...
  const handleSpeakToggle = async (text: string, idx: number) => {
    ...
    try {
      ...
      if (selectedLang !== 'en-IN') {
        const cachedTranslation = messages[idx]?.translations?.[selectedLang];
        if (cachedTranslation) {
          textToSpeak = cachedTranslation;
        } else {
          ...
          setMessages(prev => {
            ...
            if (updated[idx]) {
              updated[idx] = {
                ...updated[idx],
                translations: {
                  ...(updated[idx].translations || {}),
                  [selectedLang]: textToSpeak,
                },
              };
            }
            return updated;
      ...
      await ttsService.speak(textToSpeak, selectedLang, 'shubh', () => {
        setPlayingMsgIdx(null);
      });
      setPlayingMsgIdx(idx);
    } catch (err: any) {
      console.error('TTS Speak error:', err);
      alert(`Failed to play speech audio: ${err.message || 'Please make sure SARVAM_API_KEY is configured in your environment.'}`);
    } finally {
      setTtsLoadingIdx(null);
    }
  };
    ...
    async function fetchData() {
      ...
      try {
        const [
          ...
          api.getCollection<Exam>('exams'),
          api.getCollection<Scheme>('schemes'),
          api.getCollection<Scholarship>('scholarships'),
          api.getCollection<State>('states'),
          api.getCollection<Department>('departments'),
  ...
  const formatMessageContent = (text: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (isBulletList) {
        ...
        return (
          <div key={idx} className="flex items-start gap-2 pl-4 mt-1.5">
            ...
            <span className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed">{parseBoldText(listContent)}</span>
          </div>
        );
      }
      ...
      return (
        <p key={idx} className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mt-1">
          {parseBoldText(line)}
        </p>
      );
    });
  ...
  return (
    <div className="flex-1 bg-background relative overflow-hidden flex flex-col lg:flex-row h-[calc(100dvh-4rem)] max-h-[calc(100dvh-4rem)]">
      ...
      <div className="hidden lg:flex w-80 bg-card border-r border-border p-6 flex-col gap-6 relative z-10 shrink-0 h-full overflow-y-auto">
        <div>
        ...
        <div className="flex flex-col gap-4 bg-slate-50 dark:bg-slate-900 border border-border p-5 rounded-2xl">
          <div>
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0 relative z-10">
        ...
        {/* Banner */}
        <div className="bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100/50 dark:border-amber-900/30 p-3 text-xs font-medium text-amber-700 dark:text-amber-350 text-center flex items-center justify-center gap-2 shrink-0">
          <ShieldAlert className="h-4 w-4 shrink-0" />
          Always verify exam pattern details & deadlines with official government notification URLs before applying.
        </div>

        {/* Messages Stream */}
        <div ref={messagesContainerRef} className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/10">
          <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="flex items-end gap-2.5 w-full">
                    <div className={`p-4 rounded-2xl shadow-sm text-sm leading-relaxed border break-words ${
                      msg.role === 'user' 
                        ...
                        : 'bg-card text-foreground border-border rounded-tl-sm max-w-[90%] sm:max-w-[78%] self-start border'
                    }`}>
                      {msg.role === 'user' ? (
                        ...
                        <>
                          ...
                          {selectedLang !== 'en-IN' && msg.translations?.[selectedLang] && (
                            <div className="mt-3.5 pt-3.5 border-t border-slate-100 dark:border-slate-800/80 text-slate-700 dark:text-slate-300">
                              <div className="text-[10px] uppercase font-black tracking-wider text-primary dark:text-indigo-400 mb-1.5 flex items-center gap-1.5 select-none">
                                <Languages className="h-3.5 w-3.5" />
                                Translated ({LANGUAGES.find(l => l.code === selectedLang)?.name})
                              </div>
                              <p className="whitespace-pre-wrap font-medium">{msg.translations[selectedLang]}</p>
                            </div>
                          )}
                        </>
                      )}
                    ...
                    {msg.role === 'assistant' && (
                      <button
                        onClick={() => handleSpeakToggle(msg.content, idx)}
                        className={`p-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-primary/40 text-slate-500 hover:text-primary bg-card hover:bg-slate-50 dark:hover:bg-slate-900 transition-all cursor-pointer shadow-xs flex items-center justify-center shrink-0 self-end mb-1`}
                        title={playingMsgIdx === idx ? "Stop speaking" : "Listen to answer"}
                      >
                        {ttsLoadingIdx === idx ? (
                          <Loader2 className="h-4 w-4 animate-spin text-primary" />
                        ) : playingMsgIdx === idx ? (
                          <div className="relative flex h-4 w-4 items-center justify-center">
                            <span className="animate-ping absolute inline-flex h-3.5 w-3.5 rounded-full bg-rose-450 opacity-75"></span>
                            <VolumeX className="h-4 w-4 text-rose-500 relative" />
                          </div>
                        ) : (
                    ...
                    <div className="grid grid-cols-1 gap-5 w-full mt-2">
                      ...
                      {msg.opportunities.exams && msg.opportunities.exams.map((exam) => {
                        ...
                        return (
                          <div 
                            key={exam.id} 
                            className="bg-card border border-border rounded-2xl p-5 shadow-[0_4px_16px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 transition-all w-full flex flex-col gap-4 relative overflow-hidden group text-left"
                          >
                            ...
                            <div className="flex justify-between items-start gap-4">
                              <div>
                                <div className="flex flex-wrap items-center gap-2 mb-2">
                                  ...
                                  <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-350 text-[10px] font-bold px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
            ...
            {isLoading && (
              <div className="flex gap-3 w-full justify-start">
                ...
                <div className="bg-card border border-border p-4 rounded-2xl rounded-tl-sm flex items-center gap-3 shadow-sm shrink-0">
        ...
        <div className="p-6 border-t border-border bg-card relative z-10 shrink-0">
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="flex justify-between items-center mb-2 px-1 relative z-30">
              ...
              <div className="relative" ref={langDropdownRef}>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 text-xs font-bold text-slate-700 dark:text-slate-350 transition-all cursor-pointer shadow-xs"
                >
                ...
                {isLangDropdownOpen && (
                  <div className="absolute bottom-full right-0 mb-2 w-48 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-xl z-50 overflow-hidden py-1.5 max-h-60 overflow-y-auto">
                    {LANGUAGES.map((lang) => (
                      <button
                        key={lang.code}
                        type="button"
                        onClick={() => handleLangChange(lang.code)}
                        className={`w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold text-left hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors cursor-pointer ${
                          selectedLang === lang.code ? 'text-primary bg-primary/5' : 'text-slate-700 dark:text-slate-300'
                        }`}
                      >
            ...
            <form onSubmit={handleSend} className="flex gap-3 relative">
              <input 
                ...
                className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:bg-white dark:focus:bg-slate-950 focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-2xl pl-5 pr-14 py-3.5 text-sm outline-none transition-all shadow-inner text-slate-800 dark:text-slate-100"
              />
              <button 
                type="submit" 
                disabled={isLoading || !input.trim()}
                className="absolute right-2 top-2 bottom-2 bg-primary hover:bg-primary/95 disabled:bg-slate-200 dark:disabled:bg-slate-800 text-white w-11 flex items-center justify-center rounded-xl shadow-sm transition-colors cursor-pointer"
              >
        ...
        {selectedOpportunity && (
          <>
            ...
            <motion.div 
              ...
              className="fixed right-0 top-0 bottom-0 w-full sm:w-[480px] bg-card shadow-2xl z-[100] border-l border-border flex flex-col"
            >
              {/* Drawer Header */}
              <div className="p-6 border-b border-border bg-slate-50 dark:bg-slate-900 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl">
              ...
              {/* Action buttons */}
              <div className="p-6 border-t border-border bg-slate-50 dark:bg-slate-900 flex gap-3 shrink-0">
                {selectedOpportunity.item.official_website && (
                  <a 
                    href={selectedOpportunity.item.official_website} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="flex-1 bg-card hover:bg-slate-50/50 dark:hover:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-350 hover:text-slate-900 dark:hover:text-white py-3 rounded-xl text-xs font-bold shadow-sm transition-all flex items-center justify-center gap-1"
                  >
