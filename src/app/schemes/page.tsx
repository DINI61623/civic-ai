...
import { Search, Gift, ArrowUpRight, CheckCircle2, Loader2 } from 'lucide-react';
            ...
            schemes.map(scheme => (
              <div key={scheme.id} className="bg-card p-6 md:p-8 rounded-3xl border border-border shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all group">
                <div className="flex flex-col md:flex-row justify-between md:items-start gap-4 mb-5">
                  ...
                  <Link href={scheme.official_website || '#'} target="_blank" className="shrink-0 bg-primary/10 hover:bg-primary/20 text-primary px-5 py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center gap-1.5">
                    Visit Official Portal <ArrowUpRight className="h-4 w-4" />
                  </Link>
                </div>
