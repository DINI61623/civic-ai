...
import { 
  FALLBACK_STATES, FALLBACK_DEPARTMENTS, FALLBACK_EXAMS, FALLBACK_SCHEMES, FALLBACK_SCHOLARSHIPS,
  Exam, Scheme, Scholarship, State, Department 
} from '@/lib/fallbackData';
import { 
  ConversationFilters, parseUserQuery, filterOpportunities, getFallbackRecommendations 
...
export default function AIAssistantPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: "Namaste! I am your AI Government Career Advisor. I can help you discover which government exams, schemes, and scholarships you are eligible for.\n\nTell me about your qualification, age, state, or what career fields you are interested in!"
    }
  ]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Match department and state mappings into custom fallbacks if needed
  ...
  const submitMessage = (queryText: string) => {
    ...
    setTimeout(() => {
      // 1. Parse query and update active filters
      ...
      let replyOpportunities = { exams: [], schemes: [], scholarships: [] } as any;
      ...
      if (queryText.toLowerCase().includes('clear') || queryText.toLowerCase().includes('reset')) {
        ...
        // Defence specific response (since no defence exams exist in DB, offer fallbacks)
        isDefenceFallback = true;
        const recommendations = getFallbackRecommendations(updatedFilters, dbData);
        replyContent = "Currently, there are no specific Defence/Military exams registered in our active database. However, as a B.Tech / Graduate candidate, you are highly eligible for the following major administrative and technical central exams. Here are the top recommendations:";
        replyOpportunities = {
        ...
        const count = filterResult.exams.length + filterResult.schemes.length + filterResult.scholarships.length;
        replyContent = `Found ${count} opportunity matching your criteria:`;
      }
  ...
  // Helper to calculate status of exams
  const getOpportunityStatus = (lastDateStr: string | null) => {
    ...
    const diffTime = Math.abs(lastDate.getTime() - today.getTime());
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

    if (diffDays <= 7) {
      return { label: `Closes in ${diffDays}d`, color: 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse' };
    }
  ...
  // Suggestion buttons
  const demoPrompts = [
    { text: "Eligible after B.Tech?", label: "Eligibility after B.Tech" },
    { text: "Government jobs after Diploma?", label: "Jobs after Diploma" },
    { text: "Latest SSC notifications", label: "SSC Notifications" },
    { text: "Scholarships for engineering students", label: "Engineering Scholarships" },
    { text: "Railway jobs after Degree", label: "Railway Jobs" },
    { text: "Bank exams for graduates", label: "Bank Exams" }
  ];

  return (
    <div className="flex-1 bg-background relative overflow-hidden flex flex-col lg:flex-row">
      ...
      <div className="absolute bottom-1/4 -right-32 w-96 h-96 bg-secondary/15 rounded-full blur-[120px] pointer-events-none"></div>

      {/* Sidebar - User active profile match filters */}
      <div className="w-full lg:w-80 bg-white border-b lg:border-b-0 lg:border-r border-border p-6 flex flex-col gap-6 relative z-10 shrink-0">
        <div>
        ...
        <div className="flex flex-col gap-4 bg-slate-50 border border-border p-5 rounded-2xl">
          ...
          <div>
            <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-1.5">Reservation Category</div>
      ...
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col h-[calc(100vh-4rem)] max-h-full min-h-[500px] relative z-10">
        
        {/* Banner */}
        <div className="bg-amber-50 border-b border-amber-100/50 p-3 text-xs font-medium text-amber-700 text-center flex items-center justify-center gap-2">
          ...
          <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div key={idx} className={`flex gap-4 max-w-[85%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                ...
                <div className="space-y-4">
                  <div className={`p-4 rounded-2xl shadow-sm text-sm leading-relaxed border ${
                    msg.role === 'user' 
                      ? 'bg-white text-foreground border-slate-200 rounded-tr-sm' 
                      : 'bg-white text-foreground border-slate-200 rounded-tl-sm'
                  }`}>
                    {/* Render plain-text messages with line breaks */}
                    {msg.content.split('\n').map((line, i) => (
                      <p key={i} className={i > 0 ? 'mt-2' : ''}>{line}</p>
                    ))}
                  </div>

                  {/* Render inline cards list if opportunity feed is enabled */}
                  {msg.role === 'assistant' && msg.isOpportunityFeed && msg.opportunities && (
                    <div className="grid grid-cols-1 gap-5 w-full">
                      {/* 1. Render Exams */}
                      {msg.opportunities.exams.map((exam) => {
                        ...
                        return (
                          <div 
                            key={exam.id} 
                            className="bg-white border border-slate-200 rounded-2xl p-5 shadow-[0_4px_16px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 transition-all w-full flex flex-col gap-4 relative overflow-hidden group"
                          >
                      ...
                      {/* 2. Render Schemes */}
                      {msg.opportunities.schemes.map((scheme) => {
                      ...
                      {msg.opportunities.scholarships.map((scholarship) => {
            ...
            {isLoading && (
              <div className="flex gap-4 max-w-[85%]">
                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-primary to-secondary text-white flex items-center justify-center shrink-0 border border-primary/20">
                  <Bot className="h-5 w-5 animate-pulse" />
                </div>
                <div className="bg-white border border-slate-200 p-4 rounded-2xl rounded-tl-sm flex items-center gap-2 shadow-sm">
        ...
        <div className="p-6 border-t border-border bg-white relative z-10">
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <form onSubmit={handleSend} className="flex gap-3 relative">
              <input 
                type="text" 
                placeholder="Ask your Career Advisor: e.g. I am a B.Tech graduate age 22, tell me exams in Telangana..."
                value={input}
        ...
        {selectedOpportunity && (
          <>
            ...
            >
              ...
              <div className="p-6 border-b border-border bg-slate-50 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-2">
                  <div className={`p-2 rounded-xl text-white ${
                    ...
                    selectedOpportunity.type === 'scheme' ? 'bg-emerald-600' : 'bg-purple-600'
                  }`}>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground-muted leading-relaxed">
                    {selectedOpportunity.item.description || "Official government portal description of requirements, roles, guidelines and candidate registration information."}
                  </p>
                </div>

                {/* Selection Process / Benefits details */}
                {selectedOpportunity.type === 'exam' && (
                  <>
                    <div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {selectedOpportunity.item.preparation_resources.map((res: any, idx: number) => (
                        <a 
                          ...
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="text-xs text-primary font-bold hover:underline flex items-center justify-between bg-slate-50 hover:bg-slate-100 border border-slate-100 p-3 rounded-xl transition-all"
                        >
              ...
              {/* Apply / Official notification buttons */}
              <div className="p-6 border-t border-border bg-slate-50 flex gap-3 shrink-0">
                ...
                {selectedOpportunity.item.apply_link && (
                  <a 
                    href={selectedOpportunity.item.apply_link} 
                    target="_blank" 
                    rel="noopener noreferrer" 
